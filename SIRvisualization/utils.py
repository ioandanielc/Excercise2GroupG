...
def file_df_to_count_df(df,
    ...
    #Added the intial count for recovered individuals
    ...
    for pid in pedestrian_ids:
        ...
        for (st, g) in simtime_group:
            ...
            # If the state of the currently observed pedestrian changed from Infected to Removed (other combination would actually be impossible)
            # the pedestrian moves from being infected to being recovered, hence: subtract 1 from the total number of infected persons and
            # add one to the number of recovered persons.
            # Since no other change will be later possible, we can stop going through the stats of the currently observed pedestrian => break.
...
def create_folder_data_scatter(folder):
    ...
    scatter_i = go.Scatter(x=group_counts['simTime'],
                           ...
                           mode='lines')
    ## Create the scatter for the removed pedestrians as well.
